export function mapExperienceDTO (exp, { stripNested = true } = {}) {
    ...
    const isPublished =
        typeof exp.isPublished === 'boolean' ? exp.isPublished : true;
    ...
    const addonOptions = Array.isArray(exp.addons?.options) ? exp.addons.options : [];
    const addons = { options: addonOptions };
