...
export async function getImages ({
    ...
    const images = await prisma.image.findMany({
        ...
        orderBy,
        take: limit + 1,
        cursor: cursor && !isNaN(cursor) ? { id: Number(cursor) } : undefined,
    });
...
export async function deleteImage ({ id, userId }) {
    // Ελέγχουμε αν η εικόνα χρησιμοποιείται ως galleryImage, featuredExperience ή featuredCategory
    const image = await prisma.image.findUnique({
        where: { id },
        include: {
            galleryImages: true,         // pivot: χρησιμοποιείται σε κάποια gallery
            featuredInExperiences: true, // featured image σε Experience
            featuredInCategories: true,  // featured image σε Category
        },
    });
    ...
    if (
        ...
        image.featuredInExperiences.length > 0 ||
        image.featuredInCategories.length > 0
    ) {
